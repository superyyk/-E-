	...
	export default {
		...
		methods: {
			...
			async	alipay_Request1(){
				_self=this
					var op={
						url:'http://121.43.40.52/fmr/public/index.php/index/index/Alipay_app_car',
						data:{
							total:this.money
						},
						method:'GET',
				...
				change_order(uid,trade_no){
					uni.request({
						url:this.tp_url+'c_change_order_status',
						data:{
							uid:uid,
							trade_no:trade_no
						},
						success(res) {
							if(res.data.res==200){
								...
								setTimeout(function(){
								uni.navigateTo({
									url:'pay_result/pay_result?order_info='+JSON.stringify(res.data.data)+'&type=send' 
								})
								},1500)
